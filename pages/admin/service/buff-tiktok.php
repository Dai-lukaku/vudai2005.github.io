...
$title = "Quản lý Buff Love TikTok";
...
<div class="row">
    ...
    <div class="ibox ">
        ...
        <tbody>
            <?php 
            $query = mysqli_query($conn,"SELECT * FROM `bufftiktok` WHERE `status`= 0 ORDER BY id DESC");
            $stt = 0;
...
if($_GET['type'] === 'xoa' && $_GET['id'] && $_GET['user']){
    ...
    $check = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM `bufftiktok` WHERE `id`='$id' AND`username`='$user'")); 
    if($check){
      mysqli_query($conn,"DELETE FROM `bufftiktok` WHERE `id`='$id' AND `username`='$user'");
      echo '<script>alert("Xoá thành công , đang chuyển hướng ...");setTimeout(function(){ window.location.href = "/admin/dich-vu/buff-tiktok.html"},2000);</script>';
    }else{
  ...
  if($_GET['type'] === 'done' && $_GET['id'] && $_GET['user']){
    ...
    $check = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM `bufftiktok` WHERE `id`='$id' AND`username`='$user'")); 
    if($check){
        mysqli_query($conn,"UPDATE `bufftiktok` SET `status`= 1 WHERE  `id`= '$id' AND  `username`= '$user'");
    }
    echo '<script>alert("Done thành công , đang chuyển hướng ...");setTimeout(function(){ window.location.href = "/admin/dich-vu/buff-tiktok.html"},2000);</script>';
  }
  if($_GET['type'] === 'error' && $_GET['id'] && $_GET['user']){
    ...
    if($check){
        ...
        mysqli_query($conn,"UPDATE `bufftiktok` SET `status`= 2 WHERE `id`= '$id' AND  `username`= '$user'");
    }
